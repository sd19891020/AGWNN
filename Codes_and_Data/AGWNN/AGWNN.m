function [TrainStat,TestStat,FullStat,BatchStat,OptimalModel] = AGWNN(TrainSet,HyperParameters)
    ...
    for j=1:epochNum   
        ...
        XShuffle_vs=ShuffledSamples(1:xNum,trainNum+1:N);
        TargetShuffle_vs=ShuffledSamples(xNum+1,trainNum+1:N);
        IDSShuffle_vs=ShuffledSamples(xNum+2,trainNum+1:N);
        ...
        for jmb=1:mbs 
            ...
            %========================进行mini-batch梯度下降结束==============================
                ...
                %训练集统计指标
        if isNormalized
            [STAT_train] = FP_CV_RN(XShuffle,TargetShuffle,IDSShuffle,GW,w1,b1,w2,b2,fgw,PS_X,PS_Y);
        else
            [STAT_train] = FP_CV(XShuffle,TargetShuffle,IDSShuffle,GW,w1,b1,w2,b2,fgw);
        end
        TrainStat.loss_history =[TrainStat.loss_history,STAT_train.LOSS];
        TrainStat.rmse_history = [TrainStat.rmse_history,STAT_train.RMSE]; 
        TrainStat.r2_history = [TrainStat.r2_history,STAT_train.R2]; 
        TrainStat.r2adj_history = [TrainStat.r2adj_history,STAT_train.R2adj]; 
        TrainStat.aicc_history = [TrainStat.aicc_history,STAT_train.AICc]; 

        %验证集统计指标
        if isNormalized
            [STAT_test] = FP_CV_RN(XShuffle_vs,TargetShuffle_vs,IDSShuffle_vs,GW,w1,b1,w2,b2,fgw,PS_X,PS_Y);
        else
            [STAT_test] = FP_CV(XShuffle_vs,TargetShuffle_vs,IDSShuffle_vs,GW,w1,b1,w2,b2,fgw);
        end
        TestStat.loss_history =[TestStat.loss_history,STAT_test.LOSS];
        TestStat.rmse_history = [TestStat.rmse_history,STAT_test.RMSE]; 
        TestStat.r2_history = [TestStat.r2_history,STAT_test.R2]; 
        TestStat.r2adj_history = [TestStat.r2adj_history,STAT_test.R2adj]; 
        TestStat.aicc_history = [TestStat.aicc_history,STAT_test.AICc]; 
