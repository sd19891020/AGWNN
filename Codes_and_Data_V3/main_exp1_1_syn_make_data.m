...
[outX,outUV,outBetas,outError] = SimulatedData(ul,vl,betaMax,xMax,sigma2);
outBeta{1}=outBetas(:,[1,2,3]);%Linear & Low variance
outY{1}=outBetas(:,1)+outBetas(:,2).*outX(:,1)+outBetas(:,3).*outX(:,2)+outError;
outBeta{2}=outBetas(:,[1,2,4]);%Linear & High variance
outY{2}=outBetas(:,1)+outBetas(:,2).*outX(:,1)+outBetas(:,4).*outX(:,2)+outError;
outBeta{3}=outBetas(:,[1,2,3]);%Nonlinear & Low variance
outY{3}=outBetas(:,1)+3*tanh((outBetas(:,2).*outX(:,1))./2)+3*tanh((outBetas(:,3).*outX(:,2))./2)+outError;
outBeta{4}=outBetas(:,[1,2,4]);%Nonlinear & High variance
outY{4}=outBetas(:,1)+3*tanh((outBetas(:,2).*outX(:,1))./2)+3*tanh((outBetas(:,4).*outX(:,2))./2)+outError;
...
outY5=outBetas(:,1)+outBetas(:,2).*outX(:,1)+outBetas(:,3).*outX(:,2)+outBetas(:,4).*outX(:,3)+outError;
...
loop=1000;
for j=1:loop
    [outX,~,outBetas,err] = SimulatedData(ul,vl,betaMax,xMax,sigma2);
    Y5=outBetas(:,1)+outBetas(:,2).*outX(:,1)+outBetas(:,3).*outX(:,2)+outBetas(:,4).*outX(:,3)+err;
    DATA{j}.X=outX;
